...
MISSIONS = [
    ...
    Mission("press", "04  Die Presse", "Verarbeite einen Stahlrohling in der Presse und versende die Platte.", "Maschinen werden beladen, gestartet und nach einigen Ticks entleert.", "Liefere 1 Metallplatte.", "# Presse: (3, 2)\n# Nutze load_machine(), start_machine() und collect_output()\n", ["move", "directions", "pick_up", "drop", "machine", "load_machine", "start_machine", "collect_output", "wait"], {"delivered": {"plate": 1}}, 300),
    Mission("sensors", "05  Maschinengefuehl", "Produziere zwei Platten. Frage den Zustand ab, statt feste Wartezeiten zu raten.", "while und Sensoren machen Programme robust.", "Liefere 2 Metallplatten.", "# Beispiel:\n# while not machine_is_done():\n#     wait()\n", ["move", "directions", "pick_up", "drop", "machine", "load_machine", "start_machine", "collect_output", "wait", "conditions", "machine_is_done"], {"delivered": {"plate": 2}}, 400),
...
HELP = {
    ...
    "pick_up": ("pick_up()", "Nimmt am Materiallager oder Maschinenausgang ein Teil auf.", "if can_pick_up():\n    pick_up()"),
    "drop": ("drop()", "Legt das getragene Teil im Versand ab. Dort zaehlt es fuer den Auftrag.", "drop()"),
    ...
    "machine": ("Maschinen", "Eine Maschine nimmt passende Teile auf, verarbeitet sie und stellt ein Ergebnis bereit.", "load_machine()\nstart_machine()"),
    "load_machine": ("load_machine()", "Uebergibt das getragene Teil an die Maschine auf demselben Feld.", "load_machine()"),
    "start_machine": ("start_machine()", "Startet eine vollstaendig beladene Maschine.", "start_machine()"),
    "collect_output": ("collect_output()", "Nimmt das fertige Produkt aus der Maschine auf.", "if machine_is_done():\n    collect_output()"),
